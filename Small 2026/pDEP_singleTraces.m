...
list.id1 = [83,8550, 2886,17014,15429,3650,11446, 23885, 6227, 6909,19969];
%id2 Trapping event
list.id2 = [26606,967, 14425, 24326,31246, 31073];
% id 3 trapped
list.id3 = [9688,11764,3340,11267];
% id 4 vortex
list.id4 = [11790, 22763,15870, 6160, 22763, 25952,9323]; 
%id6 = 45 degrees
list.id5 = [32210, 21668,30453,22930,30577,2244,2496,7167,9683,16738 ];
...
for i = 1:4
subplot(1,4,i)
currentList = ['id' num2str(listidx)];

data = list.(currentList);

 
currTrace = trackRes.traces{data(i),1};
colPlot = currTrace.col;
rowPlot = currTrace.row;
zPlot   = currTrace.z;
tPlot   = (currTrace.t-currTrace.t(1))*expTime;
...
currTrace = trackRes.traces{30453,1};
colPlot = currTrace.col/200;
rowPlot = currTrace.row/200;
plot(colPlot,rowPlot);

currTrace = trackRes.traces{967,1};
colPlot = currTrace.col/200;
rowPlot = currTrace.row/200;
plot(colPlot,rowPlot);

currTrace = trackRes.traces{11764,1};
colPlot = currTrace.col/200;
rowPlot = currTrace.row/200;
plot(colPlot,rowPlot);

currTrace = trackRes.traces{11790,1};
colPlot = currTrace.col/200;
rowPlot = currTrace.row/200;
plot(colPlot,rowPlot);
 ...
 currTrace = trackRes.traces{id,1};
 col = currTrace.col/pxSize;
 row = currTrace.row/pxSize;
 z   = currTrace.z;
 t   = currTrace.t*expTime;